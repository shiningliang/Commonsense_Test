...
class SavedModel():
    ...
    def load(cls, filepath, custom_objects=None):
        ...
        # Resolves a TensorFlow bug that appears when serving in Flask and CherryPy
...
class LogisticRegressionClassifier():
    ...
    def create_model(self):
        model = Sequential()
        model.add(Dense(output_dim=self.n_output_classes, input_dim=self.n_input_nodes,
    ...
    def fit(self, seqs, labels, n_input_nodes, n_epochs=1):
        ...
        self.model.fit(seqs, labels, batch_size=self.batch_size,
                       nb_epoch=n_epochs, verbose=self.verbose)
...
class MLPClassifier():
    ...
    def create_model(self):
        ...
        for layer_num in range(self.n_hidden_layers):
            if layer_num == 0:
                model.add(Dense(output_dim=self.n_hidden_nodes, input_dim=self.n_input_nodes,
                                activation='tanh', name='hidden_layer' + str(layer_num + 1)))
            else:
                model.add(Dense(output_dim=self.n_hidden_nodes, activation='tanh',
                                name='hidden_layer' + str(layer_num + 1)))
        model.add(Dense(output_dim=self.n_output_classes,
...
class RNNClassifier():
    ...
    # , emb_weights=None, layer1_weights=None):
    def __init__(self,  n_output_classes, n_embedding_nodes=100, n_hidden_nodes=200, n_hidden_layers=1, batch_size=100, verbose=True):
        self.n_output_classes = n_output_classes
    ...
    def create_model(self):
        ...
        for layer_num in range(self.n_hidden_layers):
            ...
            model.add(GRU(output_dim=self.n_hidden_nodes, return_sequences=return_sequences,
                          name='hidden_layer' + str(layer_num + 1)))
        model.add(Dense(output_dim=self.n_output_classes,
                        activation='softmax', name='output_layer'))
    ...
    def fit(self, seqs, labels, lexicon_size, n_timesteps=None, n_epochs=1):
        ...
        if not hasattr(self, 'model'):
            ...
            if not self.n_timesteps:
                # if n_timesteps not given, set it to length of longest sequence
                self.n_timesteps = max([len(seq) for seq in seqs])
            self.model = self.create_model()
            if self.verbose:
                print("Created model", self.__class__.__name__, ":", self.__dict__)
...
class RNNLM(SavedModel):
    ...
    def __init__(self, use_features=False, use_pos=False, lexicon_size=None, n_pos_tags=None, n_timesteps=15, n_embedding_nodes=300, n_pos_embedding_nodes=25,
        ...
        '''Uncomment to use Theano for sampling'''
        # import theano
    ...
    def create_model(self, n_timesteps=None, batch_size=1, include_pred_layer=True):
        ...
        for layer_num in range(self.n_hidden_layers):
            if layer_num == 0:
                seq_hidden_layer = GRU(output_dim=self.n_hidden_nodes, return_sequences=True,
                                       stateful=True, name='seq_hidden_layer1')(seq_embedding_layer)
            else:  # add extra hidden layers
                seq_hidden_layer = GRU(output_dim=self.n_hidden_nodes, return_sequences=True,
                                       stateful=True, name='seq_hidden_layer' + str(layer_num + 1))(seq_hidden_layer)
        ...
        if self.use_pos:
            ...
            pos_hidden_layer = GRU(output_dim=self.n_pos_nodes, return_sequences=True,
                                   stateful=True, name='pos_hidden_layer')(pos_embedding_layer)

            seq_hidden_layer = merge([seq_hidden_layer, pos_hidden_layer],
                                     mode='concat', concat_axis=-1, name='pos_merge_hidden_layer')

        if self.use_features:
            feature_input_layer = Input(batch_shape=(
                batch_size, self.lexicon_size + 1), name="feature_input_layer")
            input_layers.append(feature_input_layer)
            feature_hidden_layer = Dense(
                output_dim=self.n_feature_nodes, activation='sigmoid', name='feature_hidden_layer')(feature_input_layer)
            feature_hidden_layer = RepeatVector(n_timesteps)(feature_hidden_layer)

            seq_hidden_layer = merge([seq_hidden_layer, feature_hidden_layer],
                                     mode='concat', concat_axis=-1, name='feature_merge_hidden_layer')

        output_layers = []
        if include_pred_layer:
            pred_layer = TimeDistributed(
                Dense(self.lexicon_size + 1, activation="softmax", name='pred_layer'))(seq_hidden_layer)
            output_layers.append(pred_layer)
            if self.use_pos:
                pred_pos_layer = TimeDistributed(
                    Dense(self.n_pos_tags + 1, activation="softmax", name='pred_pos_layer'))(seq_hidden_layer)
    ...
    def fit(self, seqs, pos_seqs=None, feature_vecs=None, lexicon_size=None):
        ...
        if self.verbose:
            print("training RNNLM on {} sequences with batch size = {}".format(
                len(seqs), self.batch_size))
        ...
        for batch_index in range(0, len(seqs), batch_size):
            ...
            for idx in range(max_length):  # now predict
                ...
                p_next_words = self.get_batch_p_next_words(
                    words=batch_pred_seqs[:, idx], features=batch_features)
    ...
    def get_probs(self, seqs, pos_seqs=None, feature_vecs=None, batch_size=1, return_word_probs=False):
        ...
        for batch_index in range(0, len(seqs), batch_size):
            ...
            p_next_words = [p_next_words_[-len_seq:]
                            for p_next_words_, len_seq in zip(p_next_words, len_seqs)]
...
class MLPBinaryClassifier(SavedModel):
    ...
    def create_model(self):
        ...
        for layer_idx in range(self.n_hidden_layers):
            ...
            #     return_sequences = False
            seq1_hidden_layer = Dense(output_dim=self.n_hidden_nodes,
                                      name='seq1_hidden_layer', activation='tanh')(prev_seq1_layer)
            seq2_hidden_layer = Dense(output_dim=self.n_hidden_nodes,
                                      name='seq2_hidden_layer', activation='tanh')(prev_seq2_layer)

        merge_layer = merge([seq1_hidden_layer, seq2_hidden_layer],
                            mode='concat', concat_axis=-1, name='merge_layer')
        dense_layer = Dense(output_dim=self.n_hidden_nodes,
                            name='dense_layer', activation='tanh')(merge_layer)
...
class RNNBinaryClassifier(SavedModel):
    ...
    def load(cls, filepath):
        with open(filepath + '/classifier.pkl', 'rb') as f:
            classifier = pickle.load(f)
        classifier.model = load_model(filepath + '/classifier.h5',
                                      custom_objects={'ranking_loss': classifier.ranking_loss})
        print('loaded classifier from', filepath + '/classifier.pkl')
        return classifier
...
class MLPLM(SavedModel):
    ...
    def fit(self, seqs, lexicon_size=None, n_epochs=5):
        ...
        X = [[seq[idx:idx + self.n_timesteps + 1]
              for idx in range(len(seq) - self.n_timesteps)] for seq in seqs]
    ...
    def predict(self, seqs, max_length=35, mode='max', batch_size=1, n_best=1, temp=1.0, prevent_unk=True):
        ...
        X = [[seq[idx:idx + self.n_timesteps]
              for idx in range(len(seq) - self.n_timesteps + 1)] for seq in seqs]
...
class EncoderDecoder(SavedModel):
    ...
    def create_model(self):
        ...
        if self.recurrent:  # use sequence-to-sequence (RNN) model
            ...
            emb_layer = Embedding(self.lexicon_size + 1, self.n_embedding_nodes,
                                  mask_zero=True, name='emb_seq_layer')
            ...
            decoder_dense = Dense(self.lexicon_size + 1,
                                  name='output_seq_layer', activation='softmax')
            ...
            encoded_seq_layer = Dense(output_dim=self.n_hidden_nodes,
                                      activation='sigmoid', name='encoded_seq_layer')(input_seq_layer)
            output_seq_layer = Dense(output_dim=self.lexicon_size + 1,
                                     activation='sigmoid', name='output_seq_layer')(encoded_seq_layer)
    ...
    def fit(self, seqs1, seqs2, n_timesteps=None, lexicon_size=None, n_epochs=1, save_to_filepath=False):
        ...
        if not hasattr(self, 'model'):
            ...
            if self.recurrent and not self.n_timesteps:
                # if n_timesteps not given, set it to length of longest sequence
                self.n_timesteps = max([len(seq) for seq in seqs1 + seqs2])
            self.model = self.create_model()
            if self.verbose:
                print("Created model", self.__class__.__name__, ":", self.__dict__)
    ...
    def predict(self, seq1, seq2, pred_method='multiply'):
        ...
        if self.recurrent:
            ...
            # prepend zeros (not sure if this is necessary)
            seq2 = numpy.insert(seq2, 0, numpy.zeros(len(seq2)), axis=-1)
...
class CausalEmbeddings(SavedModel):
    ...
    def create_model(self):
        ...
        if self.embedded_input:
            ...
            cause_emb_layer = Dense(output_dim=self.n_embedding_nodes,
                                    name='cause_emb_layer', activation='tanh')(cause_word_layer)
            effect_emb_layer = Dense(output_dim=self.n_embedding_nodes,
                                     name='effect_emb_layer', activation='tanh')(effect_word_layer)
        else:
    ...
    def fit(self, cause_words, effect_words, labels, lexicon_size=None, embedded_input=False, n_epochs=1):
        ...
        self.model.fit(x=[cause_words[:, None], effect_words[:, None]],
                       y=labels, batch_size=self.batch_size, nb_epoch=n_epochs)
